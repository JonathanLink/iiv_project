...
public class QuadGraph {
	...
	public void build(List<PVector> detectedLines, int width, int height) {

		int n = detectedLines.size();
		...
		for (int i = 0; i < detectedLines.size (); i++) {
			for (int j = i + 1; j < detectedLines.size (); j++) {
				if (intersect(detectedLines.get(i), detectedLines.get(j), width, height)) {  
					graph[idx][0] = i;
					graph[idx][1] = j;
					idx++;
				}
	 ...
	 */
	public  boolean isFlat(PVector c1, PVector c2, PVector c3, PVector c4) {
